...
def should_use_block_template(user: UserResponse) -> bool:
    """
    Kiểm tra xem có nên sử dụng template block hay không.
    Sử dụng block template nếu ngày hết hạn còn dưới 30 ngày.
    """
    if not user.expire:
        return False  # Nếu không có ngày hết hạn, dùng template thường
    
    # Tính số ngày còn lại
    expire_date = datetime.fromtimestamp(user.expire)
    days_remaining = (expire_date - datetime.now()).days
    
    # Sử dụng template block nếu còn dưới 30 ngày
...
def user_subscription(
    ...
    if re.match(r'^([Cc]lash-verge|[Cc]lash[-\.]?[Mm]eta|[Ff][Ll][Cc]lash|[Mm]ihomo)', user_agent):
        ...
        if LooseVersion(version_str) >= LooseVersion("6.40"):
            # Kiểm tra và chọn format phù hợp
            config_format = "block" if use_block_template else "v2ray-json"
            conf = generate_subscription(user=user, config_format=config_format, as_base64=False, reverse=False)
            return Response(content=conf, media_type="application/json", headers=response_headers)
        else:
        ...
        if LooseVersion(version_str) >= LooseVersion("1.8.29"):
            ...
            # Kiểm tra và chọn format phù hợp
            config_format = "block" if use_block_template else "v2ray-json"
            conf = generate_subscription(user=user, config_format=config_format, as_base64=False, reverse=True)
            return Response(content=conf, media_type="application/json", headers=response_headers)
        else:
            conf = generate_subscription(user=user, config_format="v2ray", as_base64=True, reverse=False)
            return Response(content=conf, media_type="text/plain", headers=response_headers)
...
def user_subscription_with_client_type(
    ...
    # Nếu client_type không tồn tại trong config, dùng v2ray-json
    if client_type not in client_config:
        client_type = "v2ray-json"
